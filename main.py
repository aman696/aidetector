...
Usage:
    ...
    python main.py --train                   Train main SVM on data/real/ and data/ai_generated/
    python main.py --train-screenshot        Train dedicated screenshot SVM
    python main.py --evaluate                Evaluate on data/test/
    python main.py --batch <directory>       Analyze all images in a directory

Notes:
    --screenshot-mode is for desktop/app UI screenshots only.
    DO NOT use it for video frame captures (Instagram Reels, TikTok, YouTube).
    Video frames share statistical properties with AI images (H.264 compression,
    no sensor noise, unimodal histogram) and will be incorrectly classified as
    AI-Generated regardless of their actual content.
"""
...
def train_model(real_dir: str = 'data/real', ai_dir: str = 'data/ai_generated',
                ...
                sm_real_dir: str = 'data/socialmediareal_ss',
                sm_ai_dir: str = 'data/socialmediaai_ss',
    ...
    # Add social media screenshots (the key dataset for fixing SM false positives)
    if os.path.isdir(sm_real_dir):
        sm_real = get_all_image_paths(sm_real_dir)
        print(f"  Adding {len(sm_real)} real social media screenshots from {sm_real_dir}")
        paths.extend(sm_real)
        labels.extend([0] * len(sm_real))

    if os.path.isdir(sm_ai_dir):
        sm_ai = get_all_image_paths(sm_ai_dir)
        print(f"  Adding {len(sm_ai)} AI social media screenshots from {sm_ai_dir}")
        paths.extend(sm_ai)
        labels.extend([1] * len(sm_ai))
    ...
    # Convert cuML model to sklearn before saving so the .pkl is GPU-free
    if use_gpu:
        from sklearn.svm import SVC as SklearnSVC
        sk_svm = SklearnSVC(kernel='rbf', probability=True, C=10.0, gamma='scale',
                            class_weight='balanced')
        sk_svm.fit(X_scaled, labels_arr)
        classifier.svm = sk_svm
        print("  Converted cuML model → sklearn (GPU-free pickle)")
...
def train_screenshot_model(
    ai_ss_dir:   str = 'data/ai_generated_screenshots',
    real_ss_dir: str = 'data/screenshots',
    sm_ai_dir:   str = 'data/socialmediaai_ss',
    sm_real_dir: str = 'data/socialmediareal_ss',
    model_path:  str = 'models/screenshot_classifier.pkl',
):
    """Train the dedicated screenshot SVM and save to disk."""
    print_banner()
    print("Training Screenshot Classifier")
    print("-" * 60)

    clf = ScreenshotClassifier()
    result = clf.train(
        ai_ss_dir, real_ss_dir,
        extra_ai_dirs=[sm_ai_dir],
        extra_real_dirs=[sm_real_dir],
        verbose=True,
    )
    clf.save(model_path)

    print(f"\nCV Accuracy:    {result['cv_accuracy_mean']:.3f} "
...
def batch_analyze(directory: str, model_path: str = 'models/svm_classifier.pkl',
    ...
    if screenshot_mode:
        print("Mode: Screenshot Classifier (dedicated 15-feature SVM)")
    print("-" * 60)
    ...
    if screenshot_mode:
        clf = ScreenshotClassifier()
        ss_model = 'models/screenshot_classifier.pkl'
        if os.path.exists(ss_model):
            clf.load(ss_model)
        else:
            print(f"Error: Screenshot model not found at {ss_model}")
            print("Run: python main.py --train-screenshot")
            sys.exit(1)
...
Examples:
  ...
  python main.py --batch data/test/real/
    ...
    parser.add_argument('--train', '-t', action='store_true',
                        help='Train main SVM on data/real/ and data/ai_generated/')
    parser.add_argument('--train-screenshot', action='store_true',
                        help='Train dedicated screenshot SVM on screenshot data')
    ...
    parser.add_argument('--screenshot-mode', '-s', action='store_true',
                        help='Use the dedicated screenshot classifier for --batch')
    ...
    if args.train:
        train_model()
    elif getattr(args, 'train_screenshot', False):
        train_screenshot_model()
    elif args.evaluate:
        ...
        if not os.path.isdir(args.batch):
            ...
            sys.exit(1)
        ss_mode = getattr(args, 'screenshot_mode', False)
        batch_analyze(args.batch, args.model, screenshot_mode=ss_mode)
    else:
        parser.print_help()
        sys.exit(1)
