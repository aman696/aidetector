...
def compute_eigenvalue_ratios(eigenvalues: np.ndarray) -> Dict[str, float]:
    ...
    # Cap ratios to prevent grayscale-as-RGB images (rank-1 covariance matrix)
    # from producing infinite condition numbers that destroy StandardScaler.
    # Empirical AI range: 4–3300. Real non-grayscale: 8–2000. Cap at 5000.
    ratios = {
        'eig_ratio_1_2': float(min(eigenvalues[0] / (eigenvalues[1] + eps), 1000.0)),
        'eig_ratio_2_3': float(min(eigenvalues[1] / (eigenvalues[2] + eps), 1000.0)),
        'eig_condition_number': float(min(eigenvalues[0] / (eigenvalues[-1] + eps), 5000.0)),
...
def patch_eigenvalue_analysis(img_bgr: np.ndarray, patch_size: int = 64) -> Dict[str, float]:
    ...
    for y in range(0, h - patch_size + 1, patch_size):
        for x in range(0, w - patch_size + 1, patch_size):
            ...
            eps = 1e-10
            patch_ratios_1_2.append(min(eigs[0] / (eigs[1] + eps), 1000.0))
